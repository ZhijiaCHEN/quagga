...
community_com2str(struct community *com)
{
    ...
    for (i = 0; i < com->size; i++)
    {
        ...
        case COMMUNITY_LOCAL_AS:
            len += strlen(" local-AS");
            break;
        case COMMUNITY_BOLERO_WITHDRAW:
            len += strlen(" Bolero-withdraw");
            break;
        default:
    ...
    for (i = 0; i < com->size; i++)
    {
        ...
        case COMMUNITY_LOCAL_AS:
            ...
            break;
        case COMMUNITY_BOLERO_WITHDRAW:
            strcpy(pnt, "Bolero-withdraw");
            pnt += strlen("Bolero-withdraw");
            break;
        default:
            as = (comval >> 16) & 0xFFFF;
            val = comval & 0xFFFF;
...
enum community_token
{
    ...
    /* BOLERO ADDED */
    community_token_bolero_withdraw,
                   ...
                   u_int32_t *val)
{
    ...
    if (isalpha((int)*p))
    {
        ...
        if (strncmp(p, "Bolero-withdraw", strlen("Bolero-withdraw")) == 0)
        {
            *val = COMMUNITY_BOLERO_WITHDRAW;
            *token = community_token_bolero_withdraw;
            p += strlen("Bolero-withdraw");
            return p;
        }
        /* Unknown string. */
