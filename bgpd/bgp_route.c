             ...
             afi_t afi, safi_t safi)
{
   ...
   */
    ...
    if (bgp_flag_check(bgp, BGP_FLAG_ALWAYS_COMPARE_MED) || (bgp_flag_check(bgp, BGP_FLAG_MED_CONFED) && confed_as_route) || aspath_cmp_left(newattr->aspath, existattr->aspath) || aspath_cmp_left_confed(newattr->aspath, existattr->aspath) || internal_as_route)
    {
...
#define NEXTHOP_IS_V6 (                              \
    ...
    /* next-hop-set */
    if (transparent || (reflect && !CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF_ALL)) || (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED) && ((NEXTHOP_IS_V4 && attr->nexthop.s_addr) || (NEXTHOP_IS_V6 && !IN6_IS_ADDR_UNSPECIFIED(&attr->extra->mp_nexthop_global)))))
    {
        /* NEXT-HOP Unchanged. */
    }
                            ...
                            struct prefix *p, struct attr *attr, afi_t afi, safi_t safi)
{
    ...
    /* next-hop-set */
    if ((p->family == AF_INET && attr->nexthop.s_addr == 0) || (p->family == AF_INET6 &&
        ...
        for (ri1 = rn->info; ri1; ri1 = ri1->next)
        {
            ...
            if (ri1->next)
                for (ri2 = ri1->next; ri2; ri2 = ri2->next)
                {
                    ...
                    if (aspath_cmp_left(ri1->attr->aspath, ri2->attr->aspath) || aspath_cmp_left_confed(ri1->attr->aspath,
                                                                                                        ri2->attr->aspath))
                    {
...
int bgp_maximum_prefix_overflow(struct peer *peer, afi_t afi,
                ...
                struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
    ...
    {
        //Each router only has a single iBGP session with Bolero, if a route is received from eBGP, pass it to Bolero
                 ...
                 struct prefix_rd *prd, u_char *tag)
{
    ...
    if ((peer->as) && (peer->as != peer->local_as))
    {
        ...
        if (PQresultStatus(bgp->boleroRes) != PGRES_COMMAND_OK)
        {
            zlog(peer->log, LOG_ERR, "Failed to withdraw route for prefix %s/%d from Bolero with SQL sentence: %s. %s\n", inet_ntoa(p->u.prefix4), p->prefixlen, sqlBuf, PQerrorMessage(bgp->boleroConn));
        }
        else
        {
                   ...
                   struct bgp_table *table, int rsclient)
{
    ...
    for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
        for (ri = rn->info; ri; ri = ri->next)
            ...
            {
                if ((rsclient) ? (bgp_announce_check_rsclient(ri, peer, &rn->p, &attr, afi, safi))
 ...
 */
int bgp_static_set_safi(safi_t safi, struct vty *vty, const char *ip_str,
...
/* Configure static BGP network. */
int bgp_static_unset_safi(safi_t safi, struct vty *vty, const char *ip_str,
                          const char *rd_str, const char *tag_str)
{
    int ret;
    struct bgp *bgp;
...
void bgp_redistribute_add(struct prefix *p, const struct in_addr *nexthop,
       ...
       */
        if ((safi == SAFI_ENCAP) || (safi == SAFI_MPLS_VPN))
        {
            if (attr->extra)
            {
                ...
                {
                case AF_INET:
                    vty_out(vty, "%s", inet_ntop(af, &attr->extra->mp_nexthop_global_in, buf, BUFSIZ));
                    break;
                case AF_INET6:
                    vty_out(vty, "%s", inet_ntop(af, &attr->extra->mp_nexthop_global, buf, BUFSIZ));
                    break;
                default:
                    vty_out(vty, "?");
                }
...
/* called from terminal list command */
void route_vty_out_tmp(struct vty *vty, struct prefix *p,
                        ...
                        afi_t afi, safi_t safi)
{
    ...
    return bgp_show(vty, NULL, afi, safi,
                    (exact ? bgp_show_type_community_list_exact : bgp_show_type_community_list), list);
}
...
/* Configuration of static route announcement and aggregate
   information. */
int bgp_config_write_network(struct vty *vty, struct bgp *bgp,
                             afi_t afi, safi_t safi, int *write)
{
    struct bgp_node *rn;
    struct prefix *p;
                              ...
                              afi_t afi, safi_t safi, int *write)
{
    ...
    if (afi == AFI_IP && safi == SAFI_UNICAST)
    {
        ...
        if (bgp->distance_ebgp && bgp->distance_ibgp && bgp->distance_local && (bgp->distance_ebgp != ZEBRA_EBGP_DISTANCE_DEFAULT || bgp->distance_ibgp != ZEBRA_IBGP_DISTANCE_DEFAULT || bgp->distance_local != ZEBRA_IBGP_DISTANCE_DEFAULT))
    ...
    {
        bgp_config_write_family_header(vty, afi, safi, write);
        if (bgp->ipv6_distance_ebgp && bgp->ipv6_distance_ibgp && bgp->ipv6_distance_local && (bgp->ipv6_distance_ebgp != ZEBRA_EBGP_DISTANCE_DEFAULT || bgp->ipv6_distance_ibgp != ZEBRA_IBGP_DISTANCE_DEFAULT || bgp->ipv6_distance_local != ZEBRA_IBGP_DISTANCE_DEFAULT))
            vty_out(vty, " distance bgp %d %d %d%s",
